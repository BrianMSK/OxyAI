...
class Ajax
{
    ...
    private SourceBundleNormalizer $sourceBundleNormalizer;
    ...
    public function __construct(
        ...
        ?SourceBundleNormalizer $sourceBundleNormalizer = null
    )
    {
        $this->requestOptions = $requestOptions ?: new RequestOptions();
        $this->sourceBundleNormalizer = $sourceBundleNormalizer ?: new SourceBundleNormalizer();
    ...
    public function handleConvert(): void
    {
        ...
        $html = $this->getSingleSourceInput();
        if (empty($html)) {
    ...
    private function getSingleSourceInput(): string
    {
        return $this->sourceBundleNormalizer->fromRequest([
            // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Raw source must remain intact for conversion.
            'html' => isset($_POST['html']) ? wp_unslash($_POST['html']) : '',
            // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Raw CSS must remain intact for conversion.
            'css' => isset($_POST['css']) ? wp_unslash($_POST['css']) : '',
            // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Raw JavaScript must remain intact for conversion.
